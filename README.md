...
- 欲探討 __機器學習（Machine Learning, ML）__ 技術在面對這份半導體產線的感測器大量數據資料集（欄位多、缺值多、分布不均）時，如將這份資料集做二元分類，分類成「良品」和「不良品」兩群，再用直觀的圖表比較，是不是就能找出導致不良的感測器或異常模式（如：群組間差異、潛在關聯、異常感測器行為）？
